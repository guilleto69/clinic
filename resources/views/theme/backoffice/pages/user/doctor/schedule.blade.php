								...
								<form class="col s12" method="post" action="{{ route('backoffice.doctor.schedule.assign', $user) }}">
									...
									<div class="row">
										<div class="input-field col m12">
											<input id="multi_date_input" name="multi_date_input" readonly="" placeholder="Seleccione los días Libres y de Vacaciones" id="days_off" type="text" class="validate">
											<label for="days_off">Días Libres</label>
										</div>
									</div>
									...
									@foreach($days as $key => $day)
										<div class="row">
											...
											<div class="col s2">
												...
												<label for="{{ $key }}-turn_a_in">Turno A Entrada Am/Pm</label>
											</div>
											<div class="col s2">
												<input id="{{ $key }}-turn_a_out" type="time" name="{{ $key }}-turn_a_out">
												<label for="{{ $key }}-turn_a_out">Turno A Salida Am/Pm</label>
											</div>
											<div class="col s2">
												<input id="{{ $key }}-turn_b_in" type="time" name="{{ $key }}-turn_b_in">
												<label for="{{ $key }}-turn_b_in">Turno B Entrada Am/Pm</label>
											</div>
											<div class="col s2">
												<input id="{{ $key }}-turn_b_out" type="time" name="{{ $key }}-turn_b_out">
												<label for="{{ $key }}-turn_b_out">Turno B Salida Am/Pm</label>
											</div>
										</div>
									@endforeach
